...
try {
    $stock_check_stmt = $conn->prepare("SELECT current_stock FROM products WHERE product_id = ?");
    foreach ($cart_items as $item) {
        ...
        if (!$row || intval($row['current_stock']) < $quantity) {
            throw new Exception("Sản phẩm " . $product_id . " không đủ tồn kho. Vui lòng kiểm tra lại số lượng.");
        }
    }
...
");
    ...
    $stock_update_stmt = $conn->prepare("UPDATE products SET current_stock = current_stock - ? WHERE product_id = ? AND current_stock >= ?");
    ...
    foreach ($cart_items as $item) {
        ...
        $unit_query = mysqli_query($conn, "SELECT unit FROM products WHERE product_id = $product_id");
        $unit_row = mysqli_fetch_assoc($unit_query);
        $unit = $unit_row['unit'] ?? null;

        $detail_stmt->bind_param("iiids", $order_id, $product_id, $quantity, $price, $unit);
        if (!$detail_stmt->execute()) {
            throw new Exception("Error inserting order detail: " . $detail_stmt->error);
        }

        $stock_update_stmt->bind_param("iii", $quantity, $product_id, $quantity);
        if (!$stock_update_stmt->execute()) {
            throw new Exception("Error updating product stock: " . $stock_update_stmt->error);
        }

        if ($stock_update_stmt->affected_rows === 0) {
            throw new Exception("Cập nhật tồn kho thất bại cho sản phẩm " . $product_id . ". Vui lòng thử lại.");
        }
    }
    ...
    mysqli_query($conn, "DELETE cd FROM cart_detail cd JOIN cart c ON cd.cart_id = c.cart_id WHERE c.user_id = $user_id");
