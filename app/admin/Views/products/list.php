...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['product_id'])) {
    ...
    // BƯỚC 1: Lấy dữ liệu cũ để bảo hiểm (An toàn cho số lượng và ảnh)
    $safeQuery = $conn->prepare("SELECT current_stock, image FROM products WHERE product_id = ?");
    $safeQuery->bind_param("i", $id);
    $safeQuery->execute();
    $oldData = $safeQuery->get_result()->fetch_assoc();
    
    $current_stock = $oldData['current_stock']; // Đảm bảo số lượng lấy từ DB, không bao giờ mất
    $db_image_path = $oldData['image']; // Mặc định dùng lại ảnh cũ

    // BƯỚC 2: Xử lý hình ảnh (Đường dẫn động)
    if (!empty($_FILES['products_image']['name'])) {
        ...
        $catStmt->execute();
        $categoryName = $catStmt->get_result()->fetch_assoc()['name'] ?? 'General';
        $folderName = str_replace(' ', '', $categoryName);

        $target_dir = realpath(__DIR__ . "/../../../../public/assets/Img/") . "/" . $folderName . "/";
        if (!file_exists($target_dir)) mkdir($target_dir, 0777, true);

        $new_filename = time() . "_" . basename($_FILES["products_image"]["name"]);
        if (move_uploaded_file($_FILES["products_image"]["tmp_name"], $target_dir . $new_filename)) {
            $db_image_path = "public/assets/Img/" . $folderName . "/" . $new_filename;
        }
    } elseif ($remove_photo == "1") {
        // Nếu người dùng nhấn nút Xóa ảnh
        $db_image_path = "public/assets/Img/default.png";
    }

    // BƯỚC 3: Cập nhật Database (Dùng 1 câu lệnh duy nhất, rất sạch sẽ)
    $stmt = $conn->prepare("UPDATE products SET name=?, status=?, category_id=?, description=?, image=?, current_stock=? WHERE product_id=?");
    $stmt->bind_param("ssissii", $name, $status, $category, $description, $db_image_path, $current_stock, $id);
...
<div class="product-grid">
    ...
    <div class="product-head">GIÁ BÁN</div>
    ...
    <?php 
        // Nếu trong DB lưu "public/assets/Img/products/abc.jpg"
        // Thì src sẽ là "../../public/assets/Img/products/abc.jpg" -> CHUẨN
        $image_src = "../../" . htmlspecialchars($row['image']); 
    ?>
    <img src="<?= $image_src ?>" width="90" height="90" 
        ...
        <div class="product-items"><?php echo number_format($row['selling_price']); ?> VNĐ</div>
    ...
    <form method="GET" action="Controllers/hidden.php" style="display:inline;" onsubmit="return confirmAction(<?= $row['current_stock'] ?>)">
        ...
        <button type="submit" class="delete-button">
            ...
            <?php else: ?>
                <?php if (strcasecmp($row['status'], 'HIDDEN') === 0): ?>
    ...
    <div id="editModal">
        <div id="editNotification" class="notification edit-notification">
            ...
            <form enctype="multipart/form-data" method="post">
                ...
                <label style="font-weight: 800; color: #333;">Số lượng tồn kho:</label>
                <input type="number" id="current_stock" name="current_stock" value="<?= $editingProduct['current_stock'] ?>" readonly tabindex="-1" style="background-color: #f5f5f5; color: #888; cursor: not-allowed; border: 1px solid #ddd;">
