...
function saveUser() {
    ...
    const isUpdate = document.getElementById('username').readOnly;
    ...
    fields.forEach(id => {
        ...
        if (element) {
            ...
            // Nếu là cập nhật và là ô mật khẩu, nếu để trống thì bỏ qua không báo lỗi
            if (isUpdate && id === 'password' && value === "") {
                return; 
            }

            formData.append(id, value);
            if (!value) {
                errors.push(`Vui lòng nhập ${fieldNames[id]}.`);
            }
        }
    });

    // 1. Kiểm tra tên đăng nhập (Regex của Thảo chuẩn rồi nè)
    const usernameVal = document.getElementById('username').value.trim();
    const usernamePattern = /^[a-z0-9_-]+$/i;
    if (usernameVal && !usernamePattern.test(usernameVal)) {
        errors.push("Tên đăng nhập không được có dấu, khoảng trắng hoặc ký tự đặc biệt.");
    }
    ...
    // 4. Kiểm tra Mật khẩu (Chỉ check độ mạnh khi thêm mới hoặc khi admin có nhập mật khẩu mới)
    const passwordElement = document.getElementById('password');
    const passwordVal = passwordElement.value.trim();
    const passwordPattern = /^(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/;
    
    if (passwordVal !== "") { // Nếu có nhập mật khẩu
        if (!passwordPattern.test(passwordVal)) {
            errors.push("Mật khẩu phải có ít nhất 8 ký tự, gồm chữ hoa, số và ký tự đặc biệt.");
        }
        const confirm_password = document.getElementById('confirm_password').value;
        if (passwordVal !== confirm_password) {
    ...
    // 5. Lấy Tên hiển thị của Thành phố và Phường
    const citySelect = document.getElementById('city');
    const wardSelect = document.getElementById('ward');
    const cityName = citySelect.options[citySelect.selectedIndex]?.text || '';
    const wardName = wardSelect.value; // Ward thường value là tên luôn
